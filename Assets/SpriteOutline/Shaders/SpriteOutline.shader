    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata_t IN)
            {
                ...
                // Calculate the surface normal (assuming 2D sprite with normals perpendicular to the surface)
                 OUT.normal = normalize(UnityObjectToWorldNormal(float3(0, 0, 1)));
            ...
            bool CheckOriginalSpriteTexture(float2 uv, bool ifZero)
            {
                ...
                else
                {
                    outline = SampleSpriteTexture(uv + fixed2(0, +thicknessY)).a > alphaThreshold ||
                        ...
                        SampleSpriteTexture(uv + fixed2(-thicknessX, 0)).a > alphaThreshold ||
                        SampleSpriteTexture(uv + fixed2(+thicknessX * cos(3.14 / 4), -thicknessY * sin(3.14 / 4))).a >
                        alphaThreshold ||
                        SampleSpriteTexture(uv + fixed2(-thicknessX * cos(3.14 / 4), +thicknessY * sin(3.14 / 4))).a >
                        alphaThreshold ||
                        SampleSpriteTexture(uv + fixed2(-thicknessX * cos(3.14 / 4), -thicknessY * sin(3.14 / 4))).a >
                ...
                for (int i = 0; i < steps; i++) // high number and not a variable to avoid stupid compiler bugs
                {
                    ...
                    if (ifZero && SampleSpriteTexture(uv + fixed2(thicknessX * cos(angle), thicknessY * sin(angle))).a
                        == 0)
                    {
                        alphaCounter++;
                    ...
                    else if (!ifZero && SampleSpriteTexture(
                        uv + fixed2(thicknessX * cos(angle), thicknessY * sin(angle))).a > alphaThreshold)
                    {
                        outline = true;
                        break;
                    }
                ...
                if (_OutlineEnabled != 0)
                {
                    ...
                    else if (_OutlineMode == 2) // Image
                    {
                        ...
                        else if (_TileMode == 1)
                        {
                            frame_coord = fixed2
                            (
                                _FrameTex_ST.x * IN.texcoord.x * _MainTex_TexelSize.z / _FrameTex_TexelSize.z -
                                _FrameTex_ST.z,
                                _FrameTex_ST.y * IN.texcoord.y * _MainTex_TexelSize.w / _FrameTex_TexelSize.w -
                                _FrameTex_ST.w
                            );
